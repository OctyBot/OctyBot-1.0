...
function initAPIListeners()
{
    ...
    API.addEventListener(API.USER_JOIN, function(user) {
        if (alerts){
            appendToChat(user.username + " joined the party!", null, "#A9D033");
            if(debug){
...
function initUIListeners()
{
    ...
    $("#plugbot-btn-autorespond").on("click", function() {
        ...
        if (!autorespond) {
            ...
            awaymsg = prompt("Going AFK? Enter a message here if you want to respond automatically to @mentions.\nAdd /user/ to the beginning of your afk message if you want to reply to the person who mentions you.","/me is AFK right now.");
            if(awaymsg != null){
                !autorespond;
...
//Changes the color of user's names in chat
    addGlobalStyle('.chat-from-featureddj, .chat-from-featureddj {color: #FE9A2E !important;}');
    addGlobalStyle('.chat-from-manager, .chat-from-manager {color: #0000FF !important;}');
    addGlobalStyle('.chat-from-cohost, .chat-from-cohost {color: #FF0000 !important;}');
    addGlobalStyle('.chat-from-host, .chat-from-host {color: #FF0000 !important;}');
...
function djAdvanced(obj) {
    setTimeout(function() {
        ...
        if ($("#button-dj-waitlist-join").css("display") === "block" && autoqueue)
            ...
            $( API.sendChat("I have Autojoin Enabled!");
    },100);
    if (userList)
        populateUserlist();
}
